...
#
	...
	print GPL_LIN_FILE <<EOF;
		...
		plot "$emp_lin_filename" t "empirical" w points, "$the_lin_filename" t "estimated" with lines lc 3, "$up_lin_filename" t "+1 std dev range" w lines lc 4, "$lo_lin_filename" t "-1 std dev range" w lines lc 4 
		quit
EOF
...
#
	...
	print GPL_EXP_FILE <<EOF;
		...
		plot "$emp_exp_filename" t "empirical" w points, "$the_exp_filename" t "theoretical" w lines lc 3
		quit
EOF
